...
With this code, the Glove Controller will start its connection to the previously informed network and does not accept updating network data through the application of this project, 
the change can be made only by changing the data in the scket of that program
-------------------------------------------------------------*/
...
#include <WiFi.h> 
WiFiClient client;
const char * host = "MASTER PC IP"; // Enter the IP of the Liquid Galaxy master computer
const char* ssid = "YOUR SSID";   // Enter this data
const char* password = "YOUR PASSWORD";// Enter this data
...
void setup()
{
  ...
  SerialBT.begin("Glove Controller"); //Bluetooth device name
       ...
       while (WiFi.status() != WL_CONNECTED) 
       {
          ...
          Serial.println("...");
          cicleTimeEnd = millis();
        if((cicleTimeEnd - cicleTimeInit)> (3000))ESP.restart();
       }
